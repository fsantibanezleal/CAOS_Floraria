...
The product uses plain Python scripts invoked by path: `run.py` orchestrates specimen and content processing, `micro.py` validates the preserved microscopic atlas, `living.py` validates the earlier living-scene extension, and `spatial.py` validates the new whole-plant graph. These are not Python distributions or internal installable packages. CPython 3.13 and its standard library provide deterministic JSON normalization, bounded HTTPS acquisition, SHA256 verification and GLB inspection. No paid service or API key is required.
...
- `data/sources/spatial-atlas.json`: eight organ regions, two routes per region, 57 bilingual scale nodes, 24 directly selectable part notes and primary source links. `spatial.py` validates references and paths and emits `data/artifacts/spatial-atlas.json` plus its exact byte/hash receipt.
...
python data-pipeline/spatial.py verify
...
`all` and `export` also generate the independent content pairs when their source files are present. `scripts/local/02_generate-data.*` includes all five canonical source JSON files in its isolated sandbox. Verification rejects partial extensions, missing translations, broken parent/child connections, unresolved sources and modified artifact bytes.
