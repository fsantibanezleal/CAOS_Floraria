...
if (Test-Path -LiteralPath $FlorariaPython) {
    & $FlorariaPython (Join-Path $FlorariaRoot 'scripts\project.py') 'package-release' @ScriptArguments
} else {
    & py -3.13 (Join-Path $FlorariaRoot 'scripts\project.py') 'package-release' @ScriptArguments
}
exit $LASTEXITCODE
