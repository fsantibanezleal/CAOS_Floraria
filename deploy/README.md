# Floraria static delivery

Floraria is a private-source application with a public anonymous static site at `https://floraria.fasl-work.com`. nginx serves an immutable artifact directory through `/var/www/<domain>/current`. There is no runtime account system, API, application process, database, provider credential or visitor login.
...
```powershell
scripts/deploy.ps1 --revision <exact-40-character-sha> --bootstrap --certificate-email <operator-email>
```

```bash
./scripts/deploy.sh --revision <exact-40-character-sha> --bootstrap --certificate-email <operator-email>
```
...
```bash
./scripts/deploy.sh --revision <exact-40-character-sha>
...
```powershell
scripts/rollback.ps1 --release-id <existing-vX.XX.XXX-revision-digest>
```

```bash
./scripts/rollback.sh --release-id <existing-vX.XX.XXX-revision-digest>
```
